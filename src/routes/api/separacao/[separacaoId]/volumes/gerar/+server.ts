...
export async function POST({ request, locals }) {
	const { nroConferencia, quantidadeAtual, quantidade } = await request.json();

	if (!locals.usuario || !quantidade) {
		return json({
			success: false,
			error: {
				title: 'Erro ao tentar gerar volumes',
				message: 'O ID do usuário é obrigatório e a quantidade é obrigatória.'
			},
			data: null
		});
	}
    ...
    try {
		const SERVICE_NAME = 'serviceName=MgeWmsSP.gerarEtiquetasVolume';
				...
				body: JSON.stringify({
                    ...
                    requestBody: {
                        ETIQUETAS: {
                            ...
                            QTDEVOLUMES: {$: quantidade},
                            QTDEXISTENTE: {$: quantidadeAtual}
                        }
                    }
				})
			}
		...
		let records = responseData.responseBody.entity;
